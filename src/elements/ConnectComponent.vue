        ...
        <div class="connect-modal-container">
          <div class="connect-modal-body">
            <div v-for="connector in availableConns" :key="connector.type">
              <template v-if="['metamask', 'metamask-mobile'].includes(connector.type)">
                ...
                <div v-else>
                  <template v-if="isMobile">
                    <a v-if="connector.href && !window.ethereum" :href="connector.href"
...
export default {
    ...
    availableConns() {
      return availableConnectors.map(el => {
        if (el.type === "metamask-mobile") {
          return {
            ...el,
            href: this.metamaskMobileDappLink,
          }
        }
        return el
      })
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      } else if (window.ethereum && window.ethereum.isMetaMask && this.isMobile) {
